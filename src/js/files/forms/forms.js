...
export function formQuantity() {
  ...
  // 		if (targetElement.closest('.quantity__button').classList.contains('quantity__button_plus')) {
  // 		  if(targetElement.closest('.quantity').querySelector('input').max > value+1){
  // 				value++;
  // 			} else  {
  // 				value = targetElement.closest('.quantity').querySelector('input').max
  // 			}
  // 		} else {
  // 			--value;
  // 			if (value < 1) value = 1;
  ...
  let quanBtns = document.querySelectorAll('.quantity__button')
  let quanInput = document.querySelector('.quantity__input input[type=number]')
  let interval = null
  let timeout = null

  if (quanBtns.length && quanInput) {
    quanBtns.forEach(quanBtn => {
      quanBtn.addEventListener("mousedown", function (e) {
        let value = parseInt(quanInput.value);
        quanInput = document.querySelector('.quantity__input input[type=number]')
        if (quanBtn.classList.contains('quantity__button_plus')) {
          if (+quanInput.max > value + 1) {
            value++;
          } else {
            value = quanInput.max
          }
          quanInput.value = value;
